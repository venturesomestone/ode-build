...
"""
Argument types useful for enforcing data-integrity and form when
parsing arguments.
"""
...
class CompilerVersion(object):
    """A wrapper type around compiler version strings."""
...
def _repr(cls, args):
    """
    A helper function for implementing __repr__ methods on *Type
    classes.
    """
...
class BoolType(object):
    """An Argument type used to validate an input string as a
    bool-like type. Callers are able to override valid true and
    false values.
    """
    ...
    def __call__(self, value):
        if value in self._true_values:
            ...
            raise ArgumentTypeError("{} isn't a boolean value".format(value))
...
class PathType(object):
    """
    PathType denotes a valid path-like object. When called paths
    will be fully expanded with the option to assert the file or
    directory referenced by the path exists.
    """
    ...
    def __call__(self, path):
        ...
        if self._assert_exists and not os.path.exists(path):
            raise ArgumentTypeError("{} doesn't exist".format(path))

        if self._assert_executable and not PathType._is_executable(path):
            raise ArgumentTypeError("{} isn't an executable".format(path))
...
class RegexType(object):
    """
    An Argument type used to validate an input string against a
    regular expression.
    """
...
class ClangVersionType(RegexType):
    """
    An Argument type used to validate Clang version strings.
    """

    ERROR_MESSAGE = (
        "Invalid version value, it must be 'MAJOR.MINOR.PATCH' or "
    ...
    def __init__(self):
        super(ClangVersionType, self).__init__(
            ClangVersionType.VERSION_REGEX, ClangVersionType.ERROR_MESSAGE)
...
# TODO Create a ergex that detect the right kind of version
# numbers
class OdeVersionType(RegexType):
    """An Argument type used to validate Ode version strings."""

    ERROR_MESSAGE = (
        "Invalid version value, it must be 'MAJOR.MINOR' or "
        "'MAJOR.MINOR.PATCH'")
    VERSION_REGEX = r"^(\d+)\.(\d+)(\.(\d+))?$"

    def __init__(self):
        super(OdeVersionType, self).__init__(
            OdeVersionType.VERSION_REGEX, OdeVersionType.ERROR_MESSAGE)

    def __call__(self, value):
        matches = super(OdeVersionType, self).__call__(value)
...
class ShellSplitType(object):
    """
    Parse and split shell arguments into a list of strings.
    Recognizes ',' as a separator as well as white spaces.
