...
"""
Hierarchy of action classes that support multiple destinations,
similar to the default actions provided by the standard argparse.
"""
...
class Nargs(object):
    """
    A container class holding valid values for the number of
    arguments an actions can accept. Other possible values
...
class Action(argparse.Action):
    """
    A slightly modified version of the Action class from argparse
    that has support for multiple destinations available rather
    than just a single one.
    """

    def __init__(
            self, option_strings, dests=None, nargs=Nargs.SINGLE, const=None,
            default=None, type=None, choices=None, required=False, help=None,
            metavar=None, dest=None):  # exists for compatibility purposes

        if dests is None and dest is None:
            raise TypeError("An action must supply either dest or dests")
        ...
        super(Action, self).__init__(
            option_strings=option_strings, dest=argparse.SUPPRESS, nargs=nargs,
    ...
    def _get_kwargs(self):
        """A method for pretty-printing out actions."""

        names = [
            "option_strings", "dests", "nargs", "const", "default", "type",
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        raise NotImplementedError("__call__ isn't defined")


# ------------------------------------------------------------- #

class AppendAction(Action):
    """An Action that is appended."""
    ...
    def __init__(self, option_strings, join=None, **kwargs):
        ...
        super(AppendAction, self).__init__(
            option_strings=option_strings, **kwargs)
...
class CustomCallAction(Action):
    """
    An Action that allows instances to implement custom call
    functionality. The call_func must follow the same calling
    convention as implementing the __call__ method for an Action.
    """
...
class StoreAction(Action):
    """An Action that stores a string."""
...
class StoreIntAction(StoreAction):
    """An Action that stores an integer."""
    ...
    def __init__(self, option_strings, **kwargs):
        ...
        # TODO Orginally 'StoreAction'
        super(StoreIntAction, self).__init__(
            option_strings=option_strings, **kwargs)


class StoreTrueAction(StoreAction):
    """
    An Action that stores True when called and False by default.
    """
...
class StoreFalseAction(StoreAction):
    """
    An Action that stores False when called and True by default.
    """
...
class StorePathAction(StoreAction):
    """
    An Action that stores a path, which it will attempt to
    expand.
    """
...
class _ToggleAction(Action):
    """
    An Action that can be toggled on or off, either implicitly
    when called or explicitly when an optional boolean value is
    parsed.
    """
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        if values is None:
            ...
            raise argparse.ArgumentTypeError(
                "{} isn't a boolean value".format(values))
...
class ToggleTrueAction(_ToggleAction):
    """
    An Action that toggles True when called or False otherwise,
    with the option to explicitly override the value.
    """

    def __init__(self, option_strings, **kwargs):

        super(ToggleTrueAction, self).__init__(
            option_strings=option_strings, on_value=True, off_value=False,
            **kwargs)


class ToggleFalseAction(_ToggleAction):
    """
    An Action that toggles False when called or True otherwise,
    with the option to explicitly override the value.
    """
    ...
    def __init__(self, option_strings, **kwargs):
        ...
        super(ToggleFalseAction, self).__init__(
            option_strings=option_strings, on_value=False, off_value=True,
            **kwargs)
...
class UnsupportedAction(Action):
    """
    An Action that denotes an unsupported argument or option and
    subsequently raises an ArgumentError.
    """
    ...
    def __init__(self, option_strings, message=None, **kwargs):
        ...
        super(UnsupportedAction, self).__init__(
            option_strings=option_strings, dests=argparse.SUPPRESS, **kwargs)
